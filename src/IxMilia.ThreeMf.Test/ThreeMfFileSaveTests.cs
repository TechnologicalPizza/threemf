...
namespace IxMilia.ThreeMf.Test
{
    ...
    {
        private static ZipArchive GetArchiveFromFile(ThreeMfFile file)
        {
        ...
        private static ZipArchiveEntry GetEntryFromFile(ThreeMfFile file, string entryPath)
        {
            using var archive = GetArchiveFromFile(file);
            return archive.GetEntry(entryPath);
        }

        private static string GetEntryText(ThreeMfFile file, string entryPath)
        {
            using var archive = GetArchiveFromFile(file);
            return GetEntryText(archive, entryPath);
        }

        private static XElement GetXmlFromFile(ThreeMfFile file, string entryPath)
        {
            var actualText = GetEntryText(file, entryPath);
            return XDocument.Parse(actualText).Root;
        }

        private static string GetEntryText(ZipArchive archive, string entryPath)
        {
...
".Trim();
        ...
        public void EnsureTexturesAreWrittenTest()
        {
            ...
            Assert.NotEqual($"/3D/Textures/{new Guid():N}.jpg", path);
        ...
        public void EnsureTextureRelationshipsArePresentTest()
        {
            var file = new ThreeMfFile();
            var model = new ThreeMfModel();
            model.Resources.Add(new ThreeMfTexture2D(Array.Empty<byte>(), ThreeMfImageContentType.Jpeg));
            model.Resources.Add(new ThreeMfTexture2D(Array.Empty<byte>(), ThreeMfImageContentType.Png));
            file.Models.Add(model);
...
".Trim();
        ...
        public void EnsureThumbnailsAreWrittenTest()
        {
            ...
            // ensure it looks correct and isn't an empty guid
            Assert.NotEqual($"{ThreeMfObject.ThumbnailPathPrefix}{new Guid():N}.jpg", path);
        ...
        public void EnsureThumbnailRelationshipsArePresentTest()
        {
            var file = new ThreeMfFile();
            var model = new ThreeMfModel();
            model.Resources.Add(new ThreeMfObject() { ThumbnailData = Array.Empty<byte>(), ThumbnailContentType = ThreeMfImageContentType.Jpeg });
            model.Resources.Add(new ThreeMfObject() { ThumbnailData = Array.Empty<byte>(), ThumbnailContentType = ThreeMfImageContentType.Png });
            file.Models.Add(model);
